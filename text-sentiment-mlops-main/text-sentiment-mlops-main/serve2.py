...
def load_model(experiment_id: str):
    """Load the model for a specific experiment ID."""
    model_path = EXPERIMENTS_DIR / experiment_id
    ...
    try:
        ...
        logger.info(f"Loaded model from {model_path} (experiment_id: {experiment_id})")
        return model, tokenizer
    except Exception as e:
        logger.error(f"Failed to load model for {experiment_id}: {str(e)}")
        raise
...
async def predict_get():
    ...
    return {
        "message": "Use POST /predict with a JSON body {'text': 'your review'} and optional query param ?experiment_id=your_experiment_id to specify a model.",
        "example": "curl -X POST http://localhost:8000/predict -d '{\"text\": \"This movie was amazing!\"}' -G -d 'experiment_id=20250516_193346'"
    }

@app.get("/experiments", response_class=HTMLResponse)
async def get_experiments(request: Request):
    """Return an HTML page with a table of experiments and a form to select an experiment for prediction."""
    try:
        ...
        <html>
            ...
            <body>
                ...
                <form action="/predict" method="post">
                    <label for="experiment_id">Select Experiment ID:</label>
                    <select name="experiment_id" id="experiment_id">
                        {''.join([f'<option value="{row["experiment_id"]}">{row["experiment_id"]} (F1: {row["eval_f1"]:.3f})</option>' for _, row in df.iterrows()])}
                    </select>
        ...
        """
        logger.info("Rendered experiment results HTML with form")
        return html_content
    except Exception as e:
...
@app.post("/predict")
async def predict(experiment_id: str = Form(...), text: str = Form(...)):
    """Predict sentiment using a specific experiment's model and log the result."""
    try:
        if not text.strip():
        ...
        model, tokenizer = load_model(experiment_id)
        ...
        logger.info(f"Prediction made: {log_entry['prediction']} (confidence: {confidence:.3f}) with model {experiment_id}")
        return {
